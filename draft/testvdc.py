...
# Imports nécessaires (adaptez selon PyQt5 / QtPy)
# from PyQt5.QtCore import Qt
# from PyQt5.QtWidgets import (
#     QWidget, QLabel, QFormLayout, QHBoxLayout, QVBoxLayout,
#     QTableWidget, QTableWidgetItem, QGroupBox, QPushButton,
#     QHeaderView, QMessageBox, QComboBox, QCheckBox
# )
# from typing import Optional, List
#
# Utilitaire attendu : set_status_pill(label: QLabel, ok: Optional[bool])
...
# Utilitaire externe attendu : set_status_pill(label: QLabel, ok: Optional[bool])
...
class SmokeDynamicPage(QWidget):
    ...
    def on_save(self) -> None:
        ...
        self.db.save_test(pid, self.TEST_KEY, ok_global, {}, payload)
        QMessageBox.information(self, "OK", "Résultat enregistré.")
...
class RecoveryPage(QWidget):
    ...
    def on_save(self):
        ...
        self.db.save_test(
            ...
            {
                "iso_class": self._last_result.get("iso_class"),
                "size_um": self._last_result.get("size_um"),
                "t_max_min": self._last_result.get("t_max_min"),
            },
            self._last_result,
        )
        QMessageBox.information(self, "OK", "Résultat enregistré.")
    ...
    def on_save(self):
        ...
        for r in range(self.table.rowCount()):
            row = {
                ...
                "fuite_non_observee": self._is_checked(r, 4),  # remplace l'ancien « reflux observé »
        ...
        # Conformité globale (mêmes règles que le live)
        any_nc = any(r["conformite"] == "Non conforme" for r in rows)
        all_ok = all(r["conformite"] == "Conforme" for r in rows if r["conformite"] in ("Conforme", "Non conforme")) and len(rows) > 0
        ok_global = False if any_nc else (True if all_ok else None)
        ...
        # Note: clé de test explicite « statique »
...
class TempRHPage(QWidget):
    ...
    def on_save(self) -> None:
        ...
        w = self._current_windows()
        any_measured = any((p.get("T") is not None and p.get("RH") is not None) for p in self.points)
        any_nc = any(not (w['Tlo'] <= p['T'] <= w['Thi'] and w['RHlo'] <= p['RH'] <= w['RHhi'])
                     for p in self.points if p.get("T") is not None and p.get("RH") is not None)
        ok_global = (False if any_nc else (True if any_measured else None))
        ...
        self.db.save_test(pid, "Temp_RH", ok_global, {}, payload)
        QMessageBox.information(self, "OK", "Résultat enregistré.")
